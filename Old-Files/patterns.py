...
def average(streaks):
    try:
        streaks_without_zeros = [streak_ for streak_ in streaks if streak_ != 0]
        return statistics.mean(streaks_without_zeros)
    except statistics.StatisticsError:
        return 'Not enough data'


def probability_list_v1_1(l1, l2, l3, streak, sno, avstlen=30):
    list_ = []
...
def csv_read():
    ...
    with open('COMMODITIES_GOLD_DATA_0007.csv', 'r') as f:
        ...
        for next_row in reader:
            ...
            if row_count >= 4:
                ...
                try:
                    ...
                    prob_list = probability_list_v1_1(for_every_1_1, for_every_1_0, for_every_1_2,
                                                      next_row[4], next_row[5])
                ...
                del row[:]
                row = [thing for thing in next_row]
        print('For loop exited at %s lines read' % row_count)
        ...
        try:
            ...
            try:
                prob_list = probability_list_v1_1(for_every_1_1, for_every_1_0, for_every_1_2, next_row[4], next_row[5])
                guess_list = guess_v1_1(prob_list)
                percent_correct = round(((guess_list[0] / (guess_list[0] + guess_list[1])) * 100), 2)
            except NameError:
                pass
        try:
            print('Num 0s, 1s, 2s:         ' + str(num_0s) + ' - ' + str(num_1s) + ' - ' + str(num_2s))
            print('Largest 0, 1, 2 Streak: ' + str(largest_0_streak) + ' - ' + str(largest_1_streak) + ' - ' + str(largest_2_streak))
            print('Num 0, 1, 2 Streaks:    ' + str(len(streaks_0)) + ' - ' + str(len(streaks_1)) + ' - ' + str(len(streaks_2)))
            try:
