...
stock_xpath = '//*[@id="cross_rate_1"]/tbody/tr[3]/td[4]'
...
def email(exc_traceback_thing):
    ...
    server.starttls()
    server.login()
    mes = """A runtime exception occurred during the running of the Commodities_SILVER.py
...
class CSVOps:
    ...
    def csv_write():
        ...
        print(values)
        with open('COMMODITIES_SILVER_DATA.csv', 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(values)

    @staticmethod
    def csv_init():
        with open('COMMODITIES_SILVER_DATA.csv', 'w') as file:
            writer = csv.writer(file)
